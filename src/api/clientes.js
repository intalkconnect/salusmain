...
router.get("/", authMiddleware, async (req, res) => {
  ...
  if (!client.is_global) {
    return res
      .status(403)
      .json({ detail: "Apenas global API key pode listar clientes" });
  }
...
router.post("/", authMiddleware, async (req, res) => {
  ...
  if (!client.is_global) {
    return res
      .status(403)
      .json({ detail: "Apenas global API key pode criar clientes" });
  }
  ...
  if (error)
    return res.status(500).json({ detail: "Erro ao criar cliente", error });
...
router.patch("/:id", authMiddleware, async (req, res) => {
  ...
  if (!client.is_global) {
    return res
      .status(403)
      .json({ detail: "Apenas global API key pode atualizar clientes" });
  }
  ...
  if (updateError)
    return res
      .status(500)
      .json({ detail: "Erro ao atualizar cliente", updateError });
  ...
  if (error)
    return res
      .status(500)
      .json({ detail: "Erro ao buscar cliente atualizado", error });
  ...
  cliente.uso_anterior = 0; // Opcional calcular como nos outros
...
router.get("/metrics", authMiddleware, async (req, res) => {
  ...
  if (!client.is_global) {
    return res
      .status(403)
      .json({ detail: "Apenas global API key pode acessar métricas" });
  }
  ...
  const avgProcessingTimeSec =
    countWithTime > 0 ? totalDuration / countWithTime : null;
